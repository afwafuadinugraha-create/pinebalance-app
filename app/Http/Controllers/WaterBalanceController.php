...
class WaterBalanceController extends Controller
{
    ...
    public function getSummaryAllPG()
    {
        ...
        $summary = $rows->groupBy('pg')->map(function ($pgRows, $pg) {
            $wpRows = $pgRows->filter(fn ($row) => (int) $row->count_wp > 0);
            $longestWp = $wpRows->sortByDesc(fn ($row) => (int) $row->count_wp)->first();

            return [
                'pg' => $pg,
                'total_lokasi' => $pgRows->count(),
                'total_hari' => $pgRows->sum('total_hari'),
                'count_fc' => $pgRows->sum('count_fc'),
                'count_fc_mad' => $pgRows->sum('count_fc_mad'),
                'count_mad_wp' => $pgRows->sum('count_mad_wp'),
                'count_wp' => $pgRows->sum('count_wp'),
                'lokasi_wp' => $wpRows->count(),
                'hari_wp_terlama' => $longestWp ? (int) $longestWp->count_wp : 0,
                'lokasi_wp_terlama' => $longestWp?->lokasi,
            ];
        })->sortBy([
            ['count_wp', 'desc'],
            ['lokasi_wp', 'desc'],
    ...
    public function getMonthlyIrrigationAllPG()
    {
        $allData = DailyWaterBalance::query()
            ->select('pg', 'tanggal', 'irigasi_mm')
            ->orderBy('tanggal')
            ->get();
        ...
        foreach ($allData as $row) {
            $monthKey = substr($row->tanggal, 0, 7);
            $allMonths[$monthKey] = true;
            $grouped[$row->pg] ??= [];
            $grouped[$row->pg][$monthKey] ??= 0;

            if ((float) $row->irigasi_mm > 0) {
                $grouped[$row->pg][$monthKey]++;
            }
        }
